...
def main():
    ...
    parser.add_argument("--image_sz", type=int, default=32, help="")
    ...
    if os.path.exists(results_path):
        if input("Resulst already obtained. Would you like to create new? (y/n)") == "y":
    ...
    # RESULTS
    columns = ['model_no', 'model_name', 'batch_size', 'learning_rate', 'pretrained', 'img_size', 'path', 'test_acc']
        ...
        for model_name in model_names:
            for learning_rate in learning_rates:
                for pretrained in pretraineds:
                    ...
                    results = model.train(train_loader=train_loader,val_loader=val_loader,epochs=epochs,learning_rate=learning_rate,test_loader=test_loader)

                    # save the model results
                    results_df.loc[len(results_df)] = [model_no, model_name, batch, learning_rate, pretrained, img_size, model_name + "_" + str(model_no), results["test_acc"]]
                    ...
                    # save the model
                    model.save_model(model_name + "_" + str(model_no) + ".pth")
